...
const HostConfigurationModal: React.FC<HostConfigurationModalProps> = ({ 
  ...
  return (
    <div className="fixed inset-0 z-50 bg-stone-900/50 backdrop-blur-lg flex items-center justify-center p-6 animate-in fade-in duration-300">
      <div className="bg-[#F7F5F0] text-charcoal border border-stone-200 w-full max-w-5xl rounded-[3rem] p-10 shadow-2xl relative my-12 text-left font-dm flex flex-col" style={{maxHeight: 'calc(100vh - 100px)'}}>
        <button onClick={onClose} className="absolute top-10 right-10 text-stone-400 hover:text-stone-800 transition-colors z-10"><X className="w-8 h-8" /></button>
        ...
        <div className="flex bg-white/50 border border-stone-200 p-2 rounded-2xl w-fit mb-12 flex-shrink-0">
          <button 
        ...
        <div className='overflow-y-auto pr-4 -mr-4'>
          <form onSubmit={handleSave} className="space-y-12">
                    ...
                    {editingHost.stripeActive && (
                        <div className="space-y-6 animate-in slide-in-from-bottom-4 duration-500">
                            <div>
                                ...
                                <input type="text" value={editingHost.stripeAccountId || ''} onChange={e => setEditingHost({...editingHost, stripeAccountId: e.target.value})} className="w-full bg-white/50 border border-stone-300 rounded-2xl p-5 text-sm focus:ring-1 focus:ring-sky-accent outline-none" placeholder="acct_..." />
                            </div>
                        </div>
                    )}
                </div>
            ...
            <div className="flex space-x-4 pt-12 border-t border-stone-200/60 flex-shrink-0">
