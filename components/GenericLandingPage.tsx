    ...
    <header className="bg-stone-950/80 backdrop-blur-lg sticky top-0 z-50 w-full border-b border-stone-800/50">
      <div className="container mx-auto px-6 py-4 flex justify-between items-center">
        ...
        <nav className="hidden md:flex items-center space-x-8">
          {navLinks.map(link => (
            <a key={link.id} href={`#${link.id}`} onClick={scrollTo(link.id)} className="text-sm font-semibold text-stone-400 hover:text-emerald-400 uppercase tracking-wider transition-colors">
              {link.label}
            </a>
          ))}
        </nav>

        <button className="md:hidden text-stone-100" onClick={() => setIsMenuOpen(!isMenuOpen)}>
          {isMenuOpen ? <X className="w-6 h-6" /> : <Menu className="w-6 h-6" />}
        </button>
      </div>
      {isMenuOpen && (
        <div className="md:hidden bg-stone-950 py-4 px-6 border-t border-stone-800/50">
          <nav className="flex flex-col space-y-4 items-center">
            {navLinks.map(link => (
              <a key={link.id} href={`#${link.id}`} onClick={scrollTo(link.id)} className="text-sm font-semibold text-stone-300 hover:text-emerald-400 py-2">
                {link.label}
              </a>
            ))}
...
const Hero: React.FC<{ onSignIn: () => void }> = ({ onSignIn }) => (
    <div className="bg-stone-950 relative text-stone-100">
        <div className="absolute inset-0 bg-grid-stone-800/20 [mask-image:linear-gradient(to_bottom,white_20%,transparent_100%)]"></div>
        <div className="container mx-auto px-6 py-24 md:py-32 text-center z-10 relative">
             <Sparkles className="mx-auto text-emerald-400 h-12 w-12 mb-4" />
            <h1 className="text-4xl md:text-6xl font-extrabold tracking-tight bg-clip-text text-transparent bg-gradient-to-b from-stone-100 to-stone-400 leading-tight mb-6">
...
const FeatureCard: React.FC<{
    ...
    <div className={`relative p-8 rounded-2xl bg-stone-900/50 border border-stone-800/60 overflow-hidden ${className}`}>
        <div className="absolute top-0 left-0 h-full w-full bg-gradient-to-tr from-emerald-900/20 via-transparent to-transparent"></div>
        <div className="relative z-10">
            <div className="inline-block bg-stone-950 text-emerald-400 rounded-lg p-3 mb-5 border border-stone-800">
                {icon}
            </div>
            <h3 className="text-xl font-bold text-stone-100 mb-3">{title}</h3>
...
const FeaturesSection: React.FC = () => (
    <section id="features" className="py-20 sm:py-28 bg-stone-950">
        <div className="container mx-auto px-4">
             <div className="text-center mb-16">
                <p className="text-emerald-400 font-bold text-lg">THE ULTIMATE TOOLKIT</p>
                <h2 className="text-4xl sm:text-5xl font-extrabold text-stone-100 tracking-tight mt-2">All-in-One Property Management</h2>
                <p className="mt-6 text-xl text-stone-400 max-w-3xl mx-auto">From five-star guest service to maximizing your revenue, Sanctum provides the tools for you to thrive independently.</p>
            </div>
...
const ROISection: React.FC = () => {
  ...
  return (
    <div className="bg-stone-950 py-20 sm:py-28" id="roi-calculator">
        <div className="container mx-auto px-4">
            <div className="text-center mb-16">
                 <p className="text-emerald-400 font-bold text-lg">RETURN ON INVESTMENT</p>
                <h2 className="text-4xl sm:text-5xl font-extrabold text-stone-100 tracking-tight">Stop Burning Money</h2>
                <p className="mt-6 text-xl text-stone-400 max-w-3xl mx-auto">Major platforms like AirBnb, booking.com and Agoda charge 15% or more. Sanctum is just 4% + Stripe Payment Fees. See what you could save by switching.</p>
            </div>

            <div className="max-w-4xl mx-auto bg-stone-900/50 rounded-2xl p-8 sm:p-12 border border-stone-800/60">
                <div className="grid md:grid-cols-2 gap-12 items-center">
                    <div>
                        <label htmlFor="revenue" className="text-lg font-bold text-stone-100 mb-2 block">Your Annual Booking Revenue</label>
                        <div className="relative">
                            <DollarSign className="absolute left-4 top-1/2 -translate-y-1/2 w-6 h-6 text-stone-500" />
                            <input
                                type="number"
                                id="revenue"
                                value={revenue}
                                onChange={(e) => setRevenue(Number(e.target.value))}
                                className="w-full bg-stone-950 border-2 border-stone-800 rounded-xl py-4 pl-14 pr-4 text-2xl font-bold text-stone-100 focus:ring-emerald-500 focus:border-emerald-500 transition"
                            />
                        </div>
                    </div>

                    <div className="text-center md:text-left">
                        <p className="text-lg text-stone-300 mb-2">Your estimated annual savings:</p>
                        <p className="text-5xl sm:text-6xl font-extrabold text-transparent bg-clip-text bg-gradient-to-r from-emerald-400 to-teal-300 tracking-tight">~${Math.round(hostSavings).toLocaleString()}</p>
                        <p className="text-md text-stone-400 mt-2">Enough for a vacation.</p>
                    </div>
                </div>
            </div>
        </div>
...
const PricingSection: React.FC<{ onSignIn: () => void }> = ({ onSignIn }) => {
  ...
  const PriceCard: React.FC<{plan: SubscriptionType, price: number, features: string[], isPopular?: boolean}> = ({ plan, price, features, isPopular }) => (
    <div className={`relative border rounded-2xl p-8 flex flex-col h-full ${isPopular ? 'border-emerald-500 ring-2 ring-emerald-500' : 'border-stone-800/60'}`}>
        {isPopular && <div className="absolute top-0 right-8 -translate-y-1/2 bg-emerald-700 text-stone-100 text-xs font-bold px-3 py-1 rounded-full uppercase">Most Popular</div>}
        <h3 className={`text-2xl font-bold ${isPopular ? 'text-emerald-400' : 'text-stone-100'}`}>{plan}</h3>
        <p className="text-5xl font-extrabold my-4 text-stone-100">
            ${price}<span className="text-lg font-medium text-stone-400">/mo</span>
        </p>
  ...
  return (
    <section id="pricing" className="py-20 sm:py-28 bg-stone-950 border-t border-stone-800">
        <div className="container mx-auto px-4">
            <div className="text-center mb-16">
                <h2 className="text-4xl sm:text-5xl font-extrabold text-stone-100 tracking-tight">Pricing That Makes Sense</h2>
                <p className="mt-6 text-xl text-stone-400 max-w-3xl mx-auto">Start small, grow big. Our success is tied to yours. All plans include our powerful direct booking engine and a 4% booking fee.</p>
            </div>
...
const DemoSection: React.FC<{ hosts: Host[] }> = ({ hosts }) => {
    ...
    return (
        <div id="demo" className="py-20 sm:py-28 bg-stone-950">
            <div className="container mx-auto px-4 text-center">
                 <h2 className="text-4xl sm:text-5xl font-extrabold text-stone-100 tracking-tight">See It in Action</h2>
                 <p className="mt-6 text-xl text-stone-300 max-w-2xl mx-auto">Words are cheap. See how Sanctum looks and feels from a guest's perspective. Select a demo host to explore their live booking site.</p>
                <div className="relative max-w-md mx-auto mt-10">
                  <select
                    id="host-select"
                    defaultValue=""
                    onChange={(e) => handleHostRedirect(e.target.value)}
                    className="bg-stone-900 border-2 border-stone-700 text-stone-100 p-5 rounded-full w-full appearance-none focus:outline-none focus:ring-2 focus:ring-emerald-500 text-center text-lg cursor-pointer"
                  >
                  ...
                  <ArrowRight className="absolute right-6 top-1/2 -translate-y-1/2 w-6 h-6 text-stone-500 pointer-events-none" />
                </div>
            </div>
        </div>
    );
    ...
    <footer className="bg-stone-950 border-t border-stone-800">
        <div className="container mx-auto px-6 py-8">
            <div className="flex flex-col md:flex-row justify-between items-center space-y-4 md:space-y-0">
                <p className="text-sm text-stone-500 text-center md:text-left">&copy; {new Date().getFullYear()} Sanctum. Own Your Bookings.</p>
                 <div className="flex items-center space-x-5">
                    <a href="#" className="text-stone-500 hover:text-emerald-400 cursor-pointer transition-colors"><Facebook className="w-5 h-5" /></a>
                    <a href="#" className="text-stone-500 hover:text-emerald-400 cursor-pointer transition-colors"><Twitter className="w-5 h-5" /></a>
                    <a href="#" className="text-stone-500 hover:text-emerald-400 cursor-pointer transition-colors"><Instagram className="w-5 h-5" /></a>
                </div>
            </div>
        </div>
    </footer>
