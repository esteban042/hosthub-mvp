...
const ApartmentCard: React.FC<ApartmentCardProps> = ({ apartment, onSelectApartment, idx }) => {
  return (
    <div 
      ...
      className="group cursor-pointer animate-in fade-in duration-700 transform transition-transform ease-in-out hover:scale-105 hover:shadow-2xl hover:shadow-stone-200/50 p-4 border border-zinc-800 hover:border-stone-200 rounded-2xl"
    ...
    >
      ...
      <div className="px-1">
        <h3 className="text-2xl font-serif font-bold text-charcoal tracking-tight mb-1 leading-none">{apartment.title}</h3>
        
        <div className="flex items-center space-x-2 text-[12px] font-bold uppercase tracking-[0.15em] text-stone-500 mb-6 mt-2">
          <div className="text-sky-accent">{CORE_ICONS.Location("w-3.5 h-3.5")}</div>
          <span>{apartment.city}</span>
        </div>

        <div className="flex items-center space-x-10">
           <div className="flex items-center space-x-2">
             <div className="text-sky-accent">{CORE_ICONS.Bed("w-4 h-4")}</div>
             <span className="text-charcoal font-medium text-sm">{apartment.bedrooms}</span>
           </div>
           <div className="flex items-center space-x-2">
             <div className="text-sky-accent">{CORE_ICONS.Bath("w-4 h-4")}</div>
             <span className="text-charcoal font-medium text-sm">{apartment.bathrooms}</span>
           </div>
           <div className="flex items-center space-x-2">
             <div className="text-sky-accent">{CORE_ICONS.Guests("w-4 h-4")}</div>
             <span className="text-charcoal font-medium text-sm">{apartment.capacity}</span>
           </div>
        </div>
      </div>
    </div>
