  ...
  return (
    <div className="space-y-10">
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
            <div>
                <label className="block text-[10px] font-black uppercase tracking-widest text-charcoal/60 mb-3">{t('host_dashboard.apartment_editor.apartment_pricing.base_price', { symbol: currency.symbol })}</label>
                <input
                    type="text"
                    inputMode="numeric"
                    pattern="[0-9]*"
                    ...
                    placeholder={t('host_dashboard.apartment_editor.apartment_pricing.base_price_placeholder')}
                    className="w-full bg-white/50 border border-stone-300 rounded-2xl p-4 text-sm text-charcoal outline-none"
                />
            </div>
            <div>
                <label className="block text-[10px] font-black uppercase tracking-widest text-charcoal/60 mb-3">{t('host_dashboard.apartment_editor.apartment_pricing.min_stay')}</label>
            ...
            <div>
                <label className="block text-[10px] font-black uppercase tracking-widest text-charcoal/60 mb-3">{t('host_dashboard.apartment_editor.apartment_pricing.max_stay')}</label>
        ...
        <div className="pt-8 border-t border-stone-200">
            <div className="flex flex-col sm:flex-row sm:items-center justify-between mb-8 gap-4">
                <h4 className="text-lg sm:text-xl font-bold text-charcoal tracking-tight">{t('host_dashboard.apartment_editor.apartment_pricing.seasonal_pricing')}</h4>
                <button type="button" onClick={onAddPriceRule} className="text-xs font-black uppercase tracking-widest bg-cyan-600/10 text-cyan-700 border border-cyan-700/40 px-6 py-2 rounded-xl hover:bg-emerald-500/20 transition-all flex items-center justify-center space-x-2">
                    ...
                    <span>{t('host_dashboard.apartment_editor.apartment_pricing.add_rate_rule')}</span>
                </button>
            </div>
            ...
            <div className="space-y-4">
                {apt.priceOverrides?.map((rule) => (
                    <div key={rule.id} className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-[1fr_1fr_auto_auto] gap-4 bg-white/50 p-6 rounded-2xl border border-stone-200 items-end animate-in slide-in-from-bottom-2">
                        <div>
                            <label className="block text-[10px] font-black uppercase text-charcoal/60 mb-2">{t('host_dashboard.apartment_editor.apartment_pricing.from_date')}</label>
                            <DatePicker
                        ...
                        <div>
                            <label className="block text-[10px] font-black uppercase text-charcoal/60 mb-2">{t('host_dashboard.apartment_editor.apartment_pricing.until_date')}</label>
                            <DatePicker
                        ...
                        <div className="grow">
                            <label className="block text-[10px] font-black uppercase text-charcoal/60 mb-2">{t('host_dashboard.apartment_editor.apartment_pricing.nightly_price', { symbol: currency.symbol })}</label>
                ...
                {(!apt.priceOverrides || apt.priceOverrides.length === 0) && (
                    <div className="py-12 border border-dashed border-stone-300 rounded-[2rem] flex flex-col items-center justify-center text-stone-400 italic text-sm text-center">
                        ...
                        <span>{t('host_dashboard.apartment_editor.apartment_pricing.no_rules_configured')}</span>
                    </div>
                )}
            </div>
        </div>
