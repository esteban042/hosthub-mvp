                  ...
                  <div key={b.id} className="w-full bg-[#1c1a19] rounded-2xl p-8 border flex flex-col md:flex-row md:items-center justify-between gap-8 transition-all hover:border-stone-700/50" style={{ borderColor: CARD_BORDER }}>
                    <div className="space-y-4 flex-1">
                       ...
                       <div className="flex items-center space-x-4">
                          <h4 className="text-2xl font-serif text-white">{b.guestEmail.split('@')[0].charAt(0).toUpperCase() + b.guestEmail.split('@')[0].slice(1)}</h4>
                    ...
                    <div className="flex items-center space-x-4">
                        {statusFilter !== 'past' && b.status === BookingStatus.REQUESTED && (
                            <button onClick={() => handleUpdateStatus(b, BookingStatus.CONFIRMED)} className="bg-white text-black px-8 py-3 rounded-xl text-[10px] font-black uppercase tracking-widest hover:bg-stone-200 transition-all active:scale-95 shadow-xl">Confirm</button>
                        )}
                        {statusFilter !== 'past' && (
                           <button onClick={() => handleUpdateStatus(b, BookingStatus.REJECTED)} className="border border-stone-500 text-stone-200 px-8 py-3 rounded-xl text-[10px] font-black uppercase tracking-widest hover:text-rose-400 hover:border-rose-500 transition-all">Reject</button>
                        )}
                    </div>
                  </div>
                ))}
