...
const AvailabilityCalendar: React.FC<{ aptId: string, bookings: Booking[], blockedDates: BlockedDate[], onToggle: (date: string) => void }> = ({ aptId, bookings, blockedDates, onToggle }) => {
  ...
  const isBlocked = (dateStr: string) => blockedDates.some(d => d.apartmentId === aptId && d.date === dateStr);
    ...
    for (let d = 1; d <= numDays; d++) {
      ...
      days.push(
        <button
          ...
          className={`h-12 flex flex-col items-center justify-center text-[10px] font-bold rounded-xl border transition-all ${
            booked 
              ? 'bg-amber-500/20 border-amber-500/40 text-amber-500' 
              : blocked 
                ? 'bg-rose-500/20 border-rose-500/40 text-rose-500' 
                : 'bg-stone-900 border-stone-800 text-stone-500 hover:text-white'
          }`}
        >
          {d}
        </button>
  ...
  return (
    <div className="pt-32 pb-24 max-w-7xl mx-auto px-6 animate-in fade-in duration-700 font-dm">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6 mb-16">
        {[
          { label: 'Managed units', value: stats.assets, icon: CORE_ICONS.Building("w-6 h-6"), color: 'text-white' },
          { label: 'Pending requests', value: stats.pending, icon: CORE_ICONS.Pending("w-6 h-6"), color: 'text-amber-500' },
          { label: 'Confirmed stays', value: stats.confirmed, icon: CORE_ICONS.Bookings("w-6 h-6"), color: 'text-emerald-500' },
          { label: 'Total revenue', value: `$${stats.revenue.toLocaleString()}`, icon: CORE_ICONS.Dollar("w-6 h-6"), color: 'text-white' }
        ].map(stat => (stat &&
      ...
      {activeTab === 'bookings' && (
        <div className="space-y-6">
           {myBookings.sort((a,b) => new Date(b.startDate).getTime() - new Date(a.startDate).getTime()).map(b => {
             ...
             return (
               <div key={b.id} className="w-full bg-[#1c1a19] rounded-2xl p-6 md:p-8 border-[1px] animate-in slide-in-from-bottom-4" style={{ borderColor: CARD_BORDER }}>
                  ...
                  <p className="text-base font-normal mb-4" style={{ color: LABEL_COLOR }}>
                    {apt?.title} – {apt?.city}
                  </p>
                  ...
                  <div className="flex flex-wrap items-center gap-x-12 gap-y-4 mb-4">
                     <div className="flex items-center space-x-3">
                        ...
                        <span className="text-white font-normal text-base">{b.startDate} – {b.endDate}</span>
                     </div>
                     <div className="flex items-center space-x-3">
                  ...
                  <div className="pt-6 border-t border-stone-800/40 flex flex-col md:flex-row md:items-center justify-between gap-6">
                     ...
                     <div className="flex space-x-3">
                        {b.status === BookingStatus.REQUESTED && (
                          <>
                             ...
                             <button onClick={() => handleUpdateStatus(b.id, BookingStatus.REJECTED)} className="border border-stone-800 text-stone-500 px-6 py-2.5 rounded-xl text-[10px] font-medium uppercase tracking-widest hover:text-white transition-all">Reject</button>
                          </>
                        )}
                        {b.status === BookingStatus.CONFIRMED && (
                          <button onClick={() => handleUpdateStatus(b.id, BookingStatus.PAID)} className="bg-emerald-500 text-white px-6 py-2.5 rounded-xl text-[10px] font-medium uppercase tracking-widest hover:bg-emerald-600 transition-all active:scale-95 shadow-lg shadow-emerald-500/20">Mark as paid</button>
                        )}
                     </div>
                  </div>
               </div>
                 ...
                 <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                    <div className="space-y-6">
                       ...
                       <div className="grid grid-cols-2 gap-4">
                          ...
                          <div>
                             <label className="block text-xs font-bold uppercase tracking-widest mb-3" style={{ color: LABEL_COLOR }}>Base Price /nt</label>
